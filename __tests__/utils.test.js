...
describe("checkArticleExists", () => {
	test("returns true if there is an article with the input id", () => {
		checkArticleExists(1).then((response) => {
			expect(response).toBe(true);
		});
	});

	test("returns custom 404 error if there is no article with the input id", () => {
		checkArticleExists(100).catch((err) => {
			expect(err).toEqual({
				status: 404,
				msg: `No article found with the id: 100`,
			});
		});
	});

	test("returns custom 400 error if the input id is not of a valid data type", () => {
		checkArticleExists("banana").catch((err) => {
			expect(err).toEqual({
				status: 400,
				msg: "Bad Request!",
			});
